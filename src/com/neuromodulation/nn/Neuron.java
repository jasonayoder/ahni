...
public class Neuron implements XmlPersistable {
     ...
     */
    // Dan - Change getValue() so that it doesn't update values when it's called, just returns
    // the value. 
    public double getValue() {
	if (dirty) {
	    if (nonIntegrating) {
    ...
    /**
     * XML representation is consistent with <a href="http://nevt.sourceforge.net/">NEVT </a>. Utility method to convert
     * collection of neurons, presumably an entire ANN, to XML.
     * 
     * @param allNeurons
     * @param outputNeurons
    ...
    /**
     * Calculates position in input layer based on average of position of inputs connected into this neuron; i.e., if a
     * this neuron has 2 input connections from input neurons at positions 2 and 3, this neurons position is 2.5.
     * 
     * @param n
     ...
     * 
     * @param neuron
     * @param allMetaData <code>Map</code> contains <code>Long</code> key innovation ID, <code>NeuronMetaData</code>
     *            value
     * @return NeuronMetaData best-guess layout position of neuron, or null if neuron is on a recursive path
     */
